...
export async function demonstrateGMTools() {
  ...
  try {
    ...
    console.log(`Simple d20 roll: ${simpleRoll.rolls[0]} (total: ${simpleRoll.finalTotal})`);
    
    const modifiedRoll = await rollDice({ dice: '1d20+5' });
    console.log(`Modified roll (1d20+5): ${modifiedRoll.rolls[0]} + 5 = ${modifiedRoll.finalTotal}`);
    
    const multipleRoll = await rollDice({ dice: '3d6' });
    console.log(`Multiple dice (3d6): [${multipleRoll.rolls.join(', ')}] = ${multipleRoll.total}`);
    
    const difficultyRoll = await rollDice({ dice: '1d20+3', difficulty: 15 });
    console.log(`Difficulty check (DC 15): ${difficultyRoll.rolls[0]} + 3 = ${difficultyRoll.finalTotal} - ${difficultyRoll.success ? 'SUCCESS' : 'FAILURE'}`);
    ...
    const knowledge1 = await storeKnowledge({
      ...
      content: 'A mystical forest where the trees seem to whisper ancient secrets. Home to friendly sprites and dangerous shadow creatures.',
    ...
    const knowledge2 = await storeKnowledge({
      category: 'npc',
      title: 'Elder Thorne',
      content: 'An ancient druid who guards the sacred grove. Has knowledge of old magics and speaks in riddles.',
    ...
    console.log(`Found ${forestKnowledge.length} forest-related knowledge entries:`);
    ...
    console.log(`GM rolls trap detection: ${trapRoll.rolls[0]} + 2 = ${trapRoll.finalTotal}`);
    
    if (trapRoll.success) {
      console.log('GM: "You notice a pressure plate hidden under the dust. You successfully avoid the trap."');
      ...
      await updateStatusTags({
        ...
        tags: [{
          name: 'Alert',
          description: 'Successfully detected a trap, +1 to next perception roll',
          type: 'buff',
          action: 'add',
          value: 1,
          duration: 120,
      ...
      console.log('GM: "You step forward confidently, but suddenly hear a click..."');
      
      // Roll for trap damage
      const trapDamage = await rollDice({ dice: '2d6' });
      console.log(`Trap damage: ${trapDamage.rolls.join(' + ')} = ${trapDamage.total} piercing damage`);
      ...
      console.log(`Status: Added "Injured" status to player (${trapDamage.total} damage)`);
    }
